...
def set_rotation():
    received_parameter, arguments = get_http_parameter(request, 'set_rotation')
    ...
    angle = r if r in allowed else 0

    sense.set_rotation(angle, redraw=redraw)
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> set_rotation({angle}, {redraw})")
    return render_template('index.html', version=version, request_log=request_log)


@app.route('/flip_h', methods=['GET', 'POST'])
def flip_h():
    received_parameter, arguments = get_http_parameter(request, 'flip_h')
    redraw = convert2Boolean(received_parameter.get('redraw'), default_value=True)

    sense.flip_h(redraw=redraw)

    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> flip_h({redraw})")
    return render_template('index.html', version=version, request_log=request_log)    


@app.route('/flip_v', methods=['GET', 'POST'])
def flip_v():
    received_parameter, arguments = get_http_parameter(request, 'flip_v')
    redraw = convert2Boolean(received_parameter.get('redraw'), default_value=True)

    sense.flip_v(redraw=redraw)

    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> flip_v({redraw})")
    return render_template('index.html', version=version, request_log=request_log)
...
def set_pixel():
    received_parameter, arguments = get_http_parameter(request, 'set_pixel')
    ...
    if 'pixel' in received_parameter:
        r, g, b = convert2RGB(received_parameter.get('pixel'), default_value=(0, 0, 0))
    else:
    ...
    if x is None or y is None:
        return f'set_pixel(): Parameter x/y fehlt oder sin nicht gültig!<br/><br/><a href="/">Back</a>', 400
    
    if 0 <= x < 8 and 0 <= y < 8:
        sense.set_pixel(x, y, r, g, b)
        request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> set_pixel({x}, {y}, {r}, {g}, {b})")
        return render_template('index.html', version=version, request_log=request_log)
    else:
        request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> set_pixel(): x/y out of range")
        return f'set_pixel(): Koordniaten x/y ausserhalb des gültigen Bereichs (0-7)!<br/><br/><a href="/">Back</a>', 400
    

@app.route('/get_pixel', methods=['GET', 'POST'])
def get_pixel():
    received_parameter, arguments = get_http_parameter(request, 'get_pixel')
    x = convert2Integer(received_parameter.get('x'), default_value=None)
    y = convert2Integer(received_parameter.get('y'), default_value=None)

    if x is None or y is None:
        return f'get_pixel(): Parameter x/y fehlt oder sin nicht gültig!<br/><br/><a href="/">Back</a>', 400
    
    if 0 <= x < 8 and 0 <= y < 8:
        r, g, b = sense.get_pixel(x, y)
        request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> get_pixel({x}, {y}) = ({r}, {g}, {b})")
        return jsonify({"x": x, "y": y, "pixel": [r, g, b]})
    else:
        request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> get_pixel(): x/y out of range")
        return f'get_pixel(): Koordniaten x/y ausserhalb des gültigen Bereichs (0-7)!<br/><br/><a href="/">Back</a>', 400
...
def show_message():
    received_parameter, arguments = get_http_parameter(request, 'show_message')
    text_string = str(received_parameter.get('text_string') or '')
    ...
    sense.show_message(text_string=text_string, scroll_speed=scroll_speed,
                       text_colour=text_colour, back_colour=back_colour)
    
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments} -> show_message('{text_string}', {scroll_speed}, {text_colour}, {back_colour})")
    return render_template('index.html', version=version, request_log=request_log)
...
def get_temperature():
    temperature = sense.get_temperature()
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: get_temperature() -> {temperature} °C")
    return jsonify({'Temperature': {'value': round(temperature, 2), 'unit': '°C'}})


@app.route('/get_pressure')
def get_pressure():
    pressure = sense.get_pressure()
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: get_pressure() -> {pressure} mBar")
    return jsonify({'Pressure': {'value': round(pressure, 2), 'unit': 'mBar'}})    


@app.route('/get_humidity')
def get_humidity():
    humidity = sense.get_humidity()
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: get_humidity() -> {humidity} %")
    return jsonify({'Humidity': {'value': round(humidity, 2), 'unit': '%'}})
...
def get_meteo_sensor_values():
    ...
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: get_meteo_sensor_values() -> {data}")
    return jsonify(data)
...
def get_weather():
    ...
    <br/><a href="/">Back</a>
    """
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: get_weather() -> {temperature} °C, {pressure} mBar, {humidity} %")
    return html
...
if __name__ == '__main__':
    app.run(debug=True, host='192.168.1.5', port=5002)  # IP-Adresse des Raspberry Pi einsetzen