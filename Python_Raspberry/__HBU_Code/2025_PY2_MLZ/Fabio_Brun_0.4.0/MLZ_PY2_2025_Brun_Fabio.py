...
# ------------------------------------------------------------------
# Name  : MLZ_PY2_2025_Brun_Fabio.py
...
#
#
# Autor: Walter Rothlin angepasst von Fabio Brun
#
# History:
...
# 27-Oct-2025  Fabio Brun         Solved Task 1
# 27-Oct-2025  Fabio Brun         Solved Task 2
# 27-Oct-2025  Fabio Brun         Integrated Class_MySenseHat_Fabio_Brun.py
# 27-Oct-2025  Fabio Brun         Imlemented Task 4 in index.html
# ------------------------------------------------------------------
...
import webcolors
import inspect
import Class_MySenseHat_Fabio_Brun as mysh

# ===========================================
# globale Variablen
# ===========================================
version = 'Fabio_Brun (V0.4.0)'
request_log = []
...
def show_message():
    ...
    scroll_speed = convert2Float(received_parameter.get('scroll_speed'), default_value=0.1)
    ...
    sense.show_message(text_string=text_string,
...
def get_temperature():
    ...
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments}")
    return jsonify({'Temperature': {'value': value, 'unit': '°C'}})
...
def get_pressure():
    ...
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments}")
    return jsonify({'Pressure': {'value': value, 'unit': 'mBar'}})
...
def get_humidity():
    ...
    request_log.append(f"{datetime.now().strftime('%d-%m-%y %H:%M:%S')}: {arguments}")
    return jsonify({'Humidity': {'value': value, 'unit': '%'}})
